...
export default function Verification() {
  ...
  // Fetch verifications with proper ordering
  const { data: allVerifications = [], loading, error } = useFirestoreCollection<any>("verifications", [orderBy("createdAt", "desc")]);

  // Filter verifications by status and role - properly sync with database
  const junkshopVerifications = allVerifications.filter(item => 
    item.userRole === 'junkshop' || 
    item.userRole === 'junk_shop_owner' || 
    item.documentType === 'business_license' ||
    item.shopName || 
  ...
  const verificationStats = {
    pending: pendingJunkshops.length,
    approved: approvedJunkshops.length,
    rejected: rejectedJunkshops.length,
    underReview: underReviewJunkshops.length,
    total: junkshopVerifications.length,
    approvalRate: junkshopVerifications.length > 0 
      ? Math.round((approvedJunkshops.length / junkshopVerifications.length) * 100)
      : 0
  };
  ...
  // Update verification status function - using direct Firestore client SDK
  const handleVerification = async (verification: any, status: 'approved' | 'rejected' | 'under_review') => {
    try {
      // Update the verification document directly in Firestore
      await updateDocument(verification.id, {
        status: status,
        reviewedBy: 'admin',
        reviewedAt: new Date(),
        rejectionReason: status === 'rejected' ? (verificationNotes || 'Rejected by admin') : null,
      ...
      alert(`Verification successfully ${status}!`);
    } catch (error) {
      console.error('Error updating verification:', error);
      alert(`Failed to ${status} verification. Please try again.`);
    }
  };
  ...
  return (
    <Layout title="Junk Shop Verification">
      <div className="space-y-8">
        ...
        {/* Debug Info */}
        {(loading || error || allVerifications.length === 0) && (
            ...
            <CardContent className="p-4">
              <div className="text-sm">
                ...
                {allVerifications.length > 0 && (
                  <div className="mt-2">
                    <div className="font-medium">Sample data structure:</div>
        ...
        <Card className="border bg-card">
          <CardHeader>
            <CardTitle className="flex items-center space-x-2">
              ...
              <span>
                {statusFilter === 'all' && `All Verifications (${filteredVerifications.length})`}
