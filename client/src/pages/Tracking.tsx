...
export default function Tracking() {
  const { data: drivers } = useFirestoreCollection<any>("users", [where("role", "==", "driver")]);
  ...
  useEffect(() => {
    const driverLocations = drivers
      .filter((driver: any) => driver.isOnline && driver.currentLocation)
      .map((driver: any) => ({
        driverId: driver.id,
  ...
  const { data: activeBookings, loading: bookingsLoading } = useFirestoreCollection<Booking>(
    "bookings",
