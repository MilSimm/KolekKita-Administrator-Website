...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
  ...
  const { data: verificationRequests } = useFirestoreCollection<any>("verification_requests", []);
  const { data: reportedContent } = useFirestoreCollection<any>("reported_content", []);
        ...
        <nav className="mt-4">
          {menuItems.map((item) => {
            ...
            if (item.id === "verification") {
              notificationCount = verificationRequests.filter((r: any) => r.status === "pending").length;
